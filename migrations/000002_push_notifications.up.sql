...
CREATE INDEX idx_push_notifications_scheduled_at ON push_notifications(scheduled_at) WHERE status = 'scheduled';
CREATE INDEX idx_push_notifications_created_by ON push_notifications(created_by);
CREATE INDEX idx_push_notifications_target_tenant ON push_notifications(target_tenant_id) WHERE target_tenant_id IS NOT NULL;
