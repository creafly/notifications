...
func (a *App) initApp() *App {
	_ = godotenv.Load()
	logger.InitFromEnv("notifications")
...
func (a *App) initHttpMiddleware() {
	...
	r.Use(middleware.HSTS(cfg.Server.GinMode == "release"))
	r.Use(middleware.ContentTypeValidation())
...
func (a *App) initHttpRouting() {
	...
	{
		protected := v1.Group("")
		protected.Use(intmiddleware.AuthMiddleware(identityClient))
		{
			...
			adminPush.Use(intmiddleware.RequireAnyClaim(identityClient, "push:manage", "push:view"))
			{
			...
			adminPushCreate.Use(intmiddleware.RequireAnyClaim(identityClient, "push:manage", "push:create"))
			{
				adminPushCreate.POST("", pushHandler.Create)
			}

			adminPushUpdate := protected.Group("/admin/push")
			adminPushUpdate.Use(intmiddleware.RequireAnyClaim(identityClient, "push:manage", "push:update"))
			{
			...
			adminPushDelete.Use(intmiddleware.RequireAnyClaim(identityClient, "push:manage", "push:delete"))
			{
